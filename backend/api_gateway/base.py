...
def _build_services() -> ServiceContainer:
    ...
    progress_for_auth = _resolve_progress_service_for_auth(settings, local_progress_service)
...
def _resolve_progress_service_for_auth(
    settings: GatewaySettings,
    local_progress_service: Any | None,
) -> StudentProgressService | FirebaseProgressService:
    if settings.progress_backend == "firebase":
        try:
            ...
            pass
    if isinstance(local_progress_service, StudentProgressService):
        return local_progress_service
...
def _build_execution_services(
    ...
    return (
        LocalExecutionService(
            progress_service=execution_progress_service,
            telemetry_service=telemetry,
        ),
        None,
    )
