...
class AuthService:
    ...
    def _backfill_auth_users(self) -> None:
        with self._lock:
            with self._database.session() as session:
                existing_ids = {row for row in session.exec(select(AuthUser.id)).all() if row}
                progress_rows = session.exec(select(StudentProgressRecord)).all()
                for record in progress_rows:
                    if record.id in existing_ids:
                        continue
                    normalized_name = _normalize_display_name(record.display_name)
                    session.add(
                        AuthUser(
                            id=record.id,
                            firebase_uid=None,
